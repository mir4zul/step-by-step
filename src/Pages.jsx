...
function Page({ eyebrow, title, children }) {
  return (
    <main id="main-content" className="mx-auto min-h-[65vh] max-w-6xl px-6 py-14 sm:py-20">
      <p className="mb-4 text-sm font-medium text-green-700 dark:text-green-400">{eyebrow}</p>
      <h1 className="mb-8 text-4xl font-semibold leading-snug">{title}</h1>
      <div className="max-w-3xl space-y-6 text-lg leading-loose text-stone-600 dark:text-stone-300">{children}</div>
    </main>
  )
}
    ...
    <Page eyebrow="পাঠের সংগ্রহ" title="আপনার পছন্দের অধ্যায় খুঁজুন">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {sections.map((section, index) => (
          <a key={section.id} href={`/#${section.id}`} className="rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 p-6 transition-colors hover:border-green-700 dark:hover:border-green-400">
            <p className="mb-2 text-xs text-green-700 dark:text-green-400">অধ্যায় {(index + 1).toLocaleString('bn-BD')}</p>
            <h2 className="text-xl font-semibold text-stone-800 dark:text-stone-100">{section.title} ↗</h2>
          </a>
        ))}
      </div>
    </Page>
...
export function Guide() {
  return (
    <Page eyebrow="সহজ নির্দেশিকা" title="নিজের ছন্দে পড়ুন">
      <h2 className="text-2xl font-semibold text-stone-800 dark:text-stone-100">ভাষা বেছে নিন</h2>
      ...
      <h2 className="text-2xl font-semibold text-stone-800 dark:text-stone-100">পছন্দের জায়গা থেকে শুরু করুন</h2>
      ...
      <h2 className="text-2xl font-semibold text-stone-800 dark:text-stone-100">বিরতি নিন, আবার ফিরে আসুন</h2>
...
export function NotFound() {
  return <Page eyebrow="৪০৪" title="এই পৃষ্ঠাটি পাওয়া যায়নি"><p>ঠিকানাটি সঠিক কি না দেখে নিন অথবা হোম পেজে ফিরে যান।</p><a href="/" className="text-green-800 dark:text-green-300 underline">হোম পেজে ফিরুন</a></Page>
}
